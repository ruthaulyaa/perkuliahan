...
import com.example.pampraktikum10_11s20018.databinding.ActivityUpdateUserBinding
...
class UpdateUserActivity : AppCompatActivity() {
    ...
    fun updateData() {
        ...
        dbRef.child(id).updateChildren(userMap).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                Toast.makeText(
                    applicationContext, "The user has been updated",
                    Toast.LENGTH_SHORT
                ).show()
                finish()
            }
