...
import com.example.pampraktikum11_11s20018.databinding.ItemImageBinding
import com.squareup.picasso.Callback
import com.squareup.picasso.Picasso
import java.lang.Exception

class MyImageAdapter(var context: Context, var myImageList: ArrayList<MyImage>):
    RecyclerView.Adapter<MyImageAdapter.UsersViewHolder>() {

    inner class UsersViewHolder(
        val adapterBinding: ItemImageBinding): RecyclerView.ViewHolder(adapterBinding.root) {}
    ...
    override fun onBindViewHolder(holder: UsersViewHolder, position: Int) {
        ...
        Picasso.get().load(imageUrl).into(holder.adapterBinding.imgData, object : Callback{
            override fun onSuccess() { holder.adapterBinding.pbData.visibility = View.INVISIBLE }
            override fun onError(e: Exception?) { Toast.makeText( context, e?.localizedMessage, Toast.LENGTH_LONG).show() }
        })
